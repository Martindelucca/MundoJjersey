...
assert.equal((home.match(/HERODESKTOP\.webp/g) ?? []).length, 1, 'Home must import one hero asset source.');
...
assert.match(heroMedia, /<Image src=\{heroImage\} alt="" widths=\{\[640, 960, 1440, 1920, 2400\]\} sizes="100vw" loading="eager" decoding="async" fetchpriority="high" \/>/);
...
assert.match(mobileProductImageRule, /background:\s*var\(--color-white\)/);
