...
assert.match(layout, /<link href="https:\/\/fonts\.googleapis\.com\/css2\?family=Barlow\+Condensed:wght@600;700;800;900&family=Sora:wght@400;500;600;700;800&display=swap" rel="preload" as="style" onload="this\.onload=null;this\.rel='stylesheet'" \/>/);
assert.match(layout, /<noscript><link href="https:\/\/fonts\.googleapis\.com\/css2\?family=Barlow\+Condensed:wght@600;700;800;900&family=Sora:wght@400;500;600;700;800&display=swap" rel="stylesheet" \/><\/noscript>/);
const layoutWithoutNoscript = layout.replace(/<noscript>[\s\S]*?<\/noscript>/g, '');
assert.doesNotMatch(layoutWithoutNoscript, /<link href="https:\/\/fonts\.googleapis\.com\/[^>]+rel="stylesheet"/);
...
assert.match(home, /import heroImage from '\.\.\/\.\.\/fotos\/HERODESKTOP\.webp'/);
assert.match(home, /<Image src=\{heroImage\} alt="" widths=\{\[640, 960, 1440, 1920, 2400\]\} sizes="100vw" loading="eager" decoding="async" fetchpriority="high" \/>/);
